        ...
        <div class="modal-body">
            <form id="editCategoryForm" action="" method="POST">
                ...
                <div class="info-box">
                    ...
                    <small>Editing this category will affect all products that belong to it.</small>
                </div>
                
                <div class="modal-buttons">
                    <button type="button" class="btn-cancel" onclick="closeEditCategoryModal()">
                        Cancel
                    </button>
...
function openEditCategoryModal(categoryId) {
    ...
    const submitBtn = document.querySelector('#editCategoryModal .btn-submit');
    ...
    .then(data => {
        if (data.success) {
            currentEditCategoryId = data.category.id;
            ...
            form.action = `/admin/categories/${data.category.id}`;
            
            clearEditCategoryErrors();
            modal.style.display = 'flex';
            document.body.style.overflow = 'hidden';
        } else {
            showNotification(data.message || 'Error loading category', 'error');
        }
    })
    .catch(error => {
...
function clearEditCategoryErrors() {
    document.querySelectorAll('#editCategoryForm .error-message').forEach(el => el.textContent = '');
    document.querySelectorAll('#editCategoryForm input, #editCategoryForm textarea').forEach(el => el.style.borderColor = '');
}
        ...
        editForm.addEventListener('submit', function(e) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    if (data.errors) showEditCategoryErrors(data.errors);
...
document.addEventListener('click', function(event) {
    const modal = document.getElementById('editCategoryModal');
    if (modal && modal.style.display === 'flex' && event.target === modal) closeEditCategoryModal();
});

document.addEventListener('keydown', function(event) {
    if (event.key === 'Escape') {
        const modal = document.getElementById('editCategoryModal');
        if (modal && modal.style.display === 'flex') closeEditCategoryModal();
    }
});
</script>